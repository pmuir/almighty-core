...
1. Enter C:\Users\YOURNAME\mercurial` as the destination directory and replace `YOURNAME` with your winows login name. Note that if you have previously installed Mercurial, you won't be prompted for a destination directory. We recommend you uninstall Mercurial using the "Add or remove programs" Windows Settings page and re-run the installer.
